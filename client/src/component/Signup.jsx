...
const Signup = ({ setAuth }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const userCred = await createUserWithEmailAndPassword(firebaseAuth, userData.email, userData.password);
      if (userCred) {
        setAuth(true);
        window.localStorage.setItem("auth", "true");
        setError(false);
        ...
        const data = await signup(userData, token);
        dispatch({
          type: actionType.SET_USER,
          user: data,
        });
        ...
        <div className="grid grid-flow-col grid-rows-3 gap-4 p-12 rounded-md shadow-xl bg-neutral-900 backdrop-blur-md lg:max-w-4xl lg:p-4 xl:max-w-6xl">
          ...
          <div className="grid col-span-2 row-span-3 pb-5 space-y-10 justify-items-stretch lg:w-96">
            ...
            <div className="w-full">
              <form method="POST" className="grid w-full gap-5 mx-auto md:w-4/5" onSubmit={handleSubmit}>
                <div className="w-full">
                ...
                <div className="flex justify-center pt-1">
                  ...
                  />
                  <label htmlFor="agreedTerm" className="ml-2 text-sm font-medium ">
                    I agree with the{" "}
                ...
                <button
                  ...
                  className="py-2 text-black transition-all duration-200 ease-in-out rounded-md hover:bg-indigo-500 hover:text-white bg-primary disabled:bg-neutral-600 disabled:text-white"
                  type="submit"
                >
